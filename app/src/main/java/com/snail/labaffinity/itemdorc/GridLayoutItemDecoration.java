...
public class GridLayoutItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        int top = position < mSpanCount && firstDivideHeight > 0 ? firstDivideHeight : 0;

        if (!isLastRaw(parent, position, mSpanCount, totalCount) || needShowLastLine) {
            if (lastDivideHeight > 0 && lastDivideHeight != mVerticalSpan && isLastRaw(parent, position, mSpanCount, totalCount)) {
                outRect.set(left, top, right, lastDivideHeight);
            } else {
                outRect.set(left, top, right, mVerticalSpan);
            }
        } else {
    ...
    public void setGapSize(int horizion, int vertical) {
    ...
    private boolean needShowLastLine;

    public void setNeedShowLastLine(boolean needShowLastLine) {
        this.needShowLastLine = needShowLastLine;
    }

    //    最后一行的高度
    private int lastDivideHeight;

    public void setLastDivideHeight(int lastDivideHeight) {
        this.lastDivideHeight = lastDivideHeight;
    }

    //    最后一行的高度
    private int firstDivideHeight;

    public void setFirstDivideHeight(int firstDivideHeight) {
        this.firstDivideHeight = firstDivideHeight;
    }
